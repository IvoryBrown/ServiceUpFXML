...
public class ClientFXMLController implements Initializable {
	...
	private boolean setClientCheckTxt() {
		if (txtClientInputClientName.getText().trim().isEmpty()) {
			cmbClientInputCounty.setPromptText("");
			txtClientInputSettlement.setStyle("-fx-prompt-text-fill: #C0C0C0");
			txtClientInputZipCode.setStyle("-fx-prompt-text-fill: #C0C0C0");
			txtClientInputMobil.setStyle("-fx-prompt-text-fill: #C0C0C0");
			txtClientInputAddress.setStyle("-fx-prompt-text-fill: #C0C0C0");
			return false;
		} else {
			return true;
		}
	...
	private void clientCheckTable() {
		...
		dataClient.addAll(ClientDataBase.getAllClient());
	}
	...
	private void btnClientCheck() {
		if (setClientCheckTxt()) {
			...
			clientCheckTable();
			dataClient.clear();
			dataClient.addAll(ClientDataBase.getClientNameFilltering(txtClientInputClientName.getText()));
